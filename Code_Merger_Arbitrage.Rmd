...
for (i in test) {
  ...
  df <- 
    data.frame(date = seq(as.POSIXct(stocks[i, 2]) - (2 * 24 * 60 * 60),
                          as.POSIXct(stocks[i,3]),
                          by="days")) %>% 
    mutate(date = as.Date(date,"%Y/%m/%d", tz = "CET")) %>% 
    mutate(day = as.numeric(as.Date(stocks[1,2], format = "%Y/%m/%d")),
